...
describe('classifyYtDlpStderr — extension hook', () => {
  it('merges extraPatterns ahead of built-ins so custom rules win', () => {
    const result = classifyYtDlpStderr('ERROR: tooManyRequests: please slow down', {
      extraPatterns: { rateLimit: /tooManyRequests/ }
    });
    expect(result.kind).toBe('rateLimit');
  });

  it('accepts an array of regexes per kind', () => {
    const result = classifyYtDlpStderr('ERROR: customSiteSpecificBan triggered', {
      extraPatterns: { ipBlock: [/anotherPattern/, /customSiteSpecificBan/] }
    });
...
describe('classifyAll', () => {
  it('returns one ClassifiedError per ERROR: line', () => {
    const raw = [
      ...
      '[download] something happens here',
