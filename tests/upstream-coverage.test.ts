  ...
  describe(`${label} — kind round-trip`, () => {
    for (const entry of snap.strings) {
      ...
      it(`[${entry.id}] "${entry.fragment.slice(0, 60)}" → ${entry.kind}`, () => {
        ...
        expect(kind, `Snapshot entry ${entry.id} declares kind="${entry.kind}" but classifier returned "${kind}"`).toBe(
          entry.kind
        );
      });
    }
...
describe('snapshot triage backlog', () => {
  it('reports kind:null entries (warning, not failure)', () => {
    ...
    if (pending.length > 0) {
      console.warn(
        `[ytdlp-errors] ${pending.length} snapshot entries have kind: null — triage before next release.`
      );
