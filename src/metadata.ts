...
export const ERROR_KIND_METADATA: Record<YtDlpErrorKind, KindMetadata> = {
  botBlock: {
    code: 'YTDLP_BOT_BLOCK',
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--cookies-from-browser', '--cookies'],
    docsUrl: `${WIKI}/Extractors#exporting-youtube-cookies`
  },
  ipBlock: {
    code: 'YTDLP_IP_BLOCK',
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--proxy', '--source-address'],
    docsUrl: `${WIKI}/FAQ#http-error-403-forbidden`
  },
  rateLimit: {
    code: 'YTDLP_RATE_LIMIT',
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--sleep-requests', '--sleep-interval', '--max-sleep-interval'],
    docsUrl: `${WIKI}/Extractors#this-content-isnt-available-try-again-later`
  },
  ageRestricted: {
    code: 'YTDLP_AGE_RESTRICTED',
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--cookies-from-browser', '--cookies'],
    docsUrl: `${WIKI}/Extractors#age-restricted-videos`
  },
  unavailable: {
  ...
  geoBlocked: {
    ...
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--geo-bypass', '--geo-bypass-country', '--proxy']
  },
  drmProtected: {
    code: 'YTDLP_DRM_PROTECTED',
    recoverable: false,
    userActionable: false,
    docsUrl: `${WIKI}/FAQ#yt-dlp-cant-download-drm-protected-content`
  },
  loginRequired: {
    code: 'YTDLP_LOGIN_REQUIRED',
    recoverable: true,
    userActionable: true,
    suggestedFlags: ['--cookies-from-browser', '--cookies', '--username', '--password']
  },
  outOfDiskSpace: {
    code: 'YTDLP_OUT_OF_DISK_SPACE',
    recoverable: true,
    userActionable: true
  },
  chunkTransferFailure: {
    code: 'YTDLP_CHUNK_TRANSFER_FAILURE',
    recoverable: true,
    userActionable: false,
  ...
  network: {
    ...
    recoverable: true,
    userActionable: false,
    suggestedFlags: ['--retries', '--socket-timeout']
  },
  unsupportedUrl: {
    code: 'YTDLP_UNSUPPORTED_URL',
    recoverable: false,
    userActionable: true,
    docsUrl: 'https://github.com/yt-dlp/yt-dlp/blob/master/supportedsites.md'
  },
  unknown: {
    code: 'YTDLP_UNKNOWN',
    recoverable: false,
